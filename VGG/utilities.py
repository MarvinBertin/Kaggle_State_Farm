"Source: https://github.com/fomorians/distracted-drivers-tf"
...
def calc_mean(scores, num_predictions):
    result = scores[0]
    for i in range(1, num_predictions):
        result += scores[i]
    result = result / float(num_predictions)
    return result

def calc_mean_arr(predictions_total, num_predictions):
    results = np.array(predictions_total[0])
    for i in range(1, num_predictions):
        results += np.array(predictions_total[i])
